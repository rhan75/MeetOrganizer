    ...
    for idx, rdr in rdr_ID_df.iterrows(): #For all races taken place for divisions and genders, iterate and get race_division_result ID
        ...
        if not ranking.empty:
            ...
            for idx, each_rank in ranking.iterrows():
                ...
                #Update DB
                engine.execute(qry)
    #rhs_ID based on meet and event
...
def update_skater(skater: dict, engine: Engine) -> None:
    ...
    if not engine.execute(exist_check).fetchall()[0][0]:
        ...
        engine.execute(qry)
# #Only works for events other than the first one. 
# #The first event will be created manually be the coach. 
# def find_skaters_for_schedule(meet_ID: int, first_division: int, last_division: int, engine:Engine) -> dict:
#     genders = [1, 2]
#     all_skaters = {}
#     for gender in genders:
#         skaters = []
#         gender_ID = gender
#         for i in range(first_division, last_division+1):
#             division_ID = i
#             qry = (
#                 f'select "skater_ID" '
#                 f'from "Meet_Skater" '
...
#                 f'and "meet_ID" = {meet_ID};'
#             )
#             skaters.append(pd.read_sql_query(qry, engine))
#         df = pd.concat(skaters)
#         all_skaters[gender_ID] = df
#     return all_skaters


#Generate list of record for the event
def generate_event_schedule_list(meet_ID: int, event_ID: int, division_range: tuple, engine: Engine) -> dict:
    event_schedule_list = {}
    genders = [1, 2]
    ...
    for gender_ID in genders:
        ...
        f'where event = {event_ID} and "meet_ID" = {meet_ID} and "division_ID" between {low_div} and {high_div} and skater."gender_ID" = {gender_ID} order by time desc;'
        )
        skaters = pd.read_sql_query(qry, engine)
        event_schedule_list[gender_ID] = skaters
    return event_schedule_list


#Query to find skater_IDs of the division in between 3 and 6, for male and meet_ID = 1
#To use this, need to populate Meet_Skater using update_meet_skater 
'''
(select "skater_ID" 
from "Meet_Skater" as ms where ("division_ID" between 3 and 6) and "gender_ID" = 1 and "meet_ID" =1 )
'''

#Query to get seed time of skaters
'''
select skater.club_member_number as "Skater Number", skater.first_name as "First Name", 
skater.last_name as "Last Name", skater.seed_speed as "Speed"  from (select "skater_ID" 
from "Meet_Skater" as ms where ("division_ID" between 3 and 6) and "gender_ID" = 1 and "meet_ID" =1 ) as male 
left join "Skater" as skater on male."skater_ID" = skater."skater_ID" ;
'''

'''
To construct "Meet_Skater" for a meet from a list of skaters, 
from skater's list, iterate over each skater,
if the skater exists: 
    get the skater_ID, gender_ID and dob, use dob to get skater's age, then use the age to get the division info
    using skater_ID, Meet_ID, gender_ID, and division_ID, update Meet_Skater table
else:
    output report of missing skater, import skater info and update "Skater" table or 
    from Skater, get skater_ID, gender_ID, dob, use dob to get skater's age, then use the age to get the division info
    using skater_ID, meet_ID, gender_ID, and division_ID, update Meet_Skater table 
...
#         )
        ...
        #     """
        # html = title + html + end_html
        # html_file = f'{report_path}/division_event_{event}_report{idx}.html'
        # pdf_file = f'{report_path}/division_event_{event}_report{idx}.pdf'
        # with open(html_file, 'w') as file:
        #     file.write(html)
        # HTML(html_file).write_pdf(pdf_file)
...
def generate_race_heat_report(meet_ID: int, event: int, report_path: str, engine: Engine) -> None:
    ...
    for idx, row in rhs_IDs.iterrows(): 
        ...
        #     """
        # html = title + html + end_html
        # html_file = f'{report_path}/heat_result-event-{event}-{idx}.html'
        # pdf_file = f'{report_path}/heat_result-event-{event}-{idx}.pdf'
        # with open(html_file, 'w') as file:
        #     file.write(html)
        # HTML(html_file).write_pdf(pdf_file)
...
def generate_meet_division_report(meet_ID: int, report_path: str, engine: Engine) -> None:
    ...
    for idx, each_mdr in mdr.iterrows():
        ...
        #     """
        # html = title + html + end_html
        # html_file = f'{report_path}/meet_division_report{idx}.html'
        # pdf_file = f'{report_path}/meet__division_report{idx}.pdf'
        # with open(html_file, 'w') as file:
        #     file.write(html)
        # HTML(html_file).write_pdf(pdf_file)


# def generate_heat_report(meet_ID: int, engine: Engine) -> None:
#     #find rdr_ID using meet_ID, division_ID, race_ID, gender_ID
